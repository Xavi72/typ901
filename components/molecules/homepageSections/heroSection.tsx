...
const HeroSection = ({ dict }: { dict: any }) => {
  return (
    <section className="flex flex-row w-full h-full justify-center relative">
      ...
      />
      <div className="absolute bottom-32 flex w-full h-auto px-[2%] justify-center z-30">
        <div className="flex flex-row flex-wrap w-full max-w-[1000px] h-auto gap-x-4 text-center justify-center">
          {dict?.landing?.hero.title.map((title: string, index: number) => (
            <div
              ...
              key={index}
            >
              <h1 className="font-typ901 text-white text-[20px] sm:text-[32px] md:text-[48px] text-center  tracking-widest">
                {title}
              </h1>
            </div>
          ))}
